...
import { getWebsiteConfig, type WebsiteConfig } from "@/services/supabase/queries";

const FALLBACK: Partial<WebsiteConfig> = {
  address: "Jl. Raya Semplak Jl. Cemplang Utara No.276\nCilendek Bar., Bogor Barat, Jawa Barat 16114",
  phone: "(0251) 123-4567\n+62 812-3456-7890",
  email: "info@smppgri8bogor.sch.id\nppdb@smppgri8bogor.sch.id",
};

const Contact = () => {
  const [config, setConfig] = useState<Partial<WebsiteConfig>>(FALLBACK);
  ...
  const contactItems = [
    { icon: MapPin, t: "Alamat", d: config.address ?? FALLBACK.address! },
    { icon: Phone, t: "Telepon", d: config.phone ?? FALLBACK.phone! },
    { icon: Mail, t: "Email", d: config.email ?? FALLBACK.email! },
  ];
      ...
      <div className="container-eduka">
        ...
        <div className="grid md:grid-cols-3 gap-4 mb-10">
          {contactItems.map((c, idx) => (
            <ScrollReveal key={c.t} animation="slide-up" delay={idx * 100}>
              <div className="bg-card rounded-2xl p-6 border border-border shadow-card hover:shadow-elevated md:hover:scale-105 transition-all duration-300 h-full">
        ...
        <div className="grid lg:grid-cols-3 gap-6">
          ...
          <ScrollReveal animation="slide-left">
            <div className="bg-primary text-primary-foreground rounded-2xl p-8 shadow-card flex flex-col justify-center md:hover:scale-105 transition-transform duration-300">
