    ...
    <header className="sticky top-0 z-50 bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/80 border-b border-border shadow-soft">
      <div className="container-eduka flex items-center justify-between h-16 md:h-20">
        ...
        <div className="hidden md:flex items-center gap-2 md:gap-3">
          {!isPPDBPage && (
            <Button variant="cta" size="sm" md:size="lg" asChild>
      ...
      <div className={cn(
        ...
        <nav className="container-eduka flex flex-col py-3 md:py-4 gap-1">
          {links.map((l) => (
            <a key={l.href} href={l.href} onClick={() => setOpen(false)}
              className="px-3 md:px-4 py-2.5 md:py-3 rounded-md text-sm font-medium hover:text-primary transition-colors">
              {l.label}
            </a>
          ))}
          {!isPPDBPage && (
            <Button variant="cta" size="sm" md:size="lg" className="mt-2" asChild>
